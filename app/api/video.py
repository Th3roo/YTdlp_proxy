...
async def read_file_chunked(file_path: str, start: int, end: int):
    """Асинхронно читает и отдает чанки локального файла."""
    ...
    try:
        ...
        # Этот генератор не должен выбрасывать исключения, просто прекращает отдачу
        return
...
async def stream_live_video(
    ...
    """
    Стримит видео из очереди. Может стримить как локальный файл (если скачан),
    так и удаленный URL.
    """
    active_video_url, active_video_title = queue_manager.get_current_video_details_for_stream()

    # Если URL есть и это существующий локальный файл
    if active_video_url and os.path.exists(active_video_url):
        try:
            ...
            headers = {
                "Accept-Ranges": "bytes",
                "Content-Type": "video/mp4", # Для простоты, можно расширить
                "X-Stream-Title": active_video_title.encode('utf-8', 'surrogateescape').decode('latin-1', 'replace'),
            ...
            return StreamingResponse(
                read_file_chunked(active_video_url, start_byte, end_byte),
                status_code=status_code,
                headers=headers,
            )
        except Exception as e:
            logger.error(f"Ошибка стриминга локального файла {active_video_url}: {e}", exc_info=True)
            active_video_url = None # Переходим к заглушке
    
    # Если это удаленный URL
        ...
        try:
            ...
            # Переходим к заглушке
    
    # Если стримить нечего (URL не было, или произошла ошибка), показываем заглушку
    if os.path.exists(PLACEHOLDER_VIDEO_PATH):
        ...
        return StreamingResponse(
            read_file_chunked(PLACEHOLDER_VIDEO_PATH, start_byte, end_byte),
            status_code=status_code,
            headers=headers
        )
...
async def add_video_to_queue(payload: VideoCreate = Body(...)):
    """
    Добавляет видео в очередь. Сразу получает метаданные и проверяет наличие файла.
    """
    try:
    ...
    if new_video_entry.status == "downloaded":
        message = "Видео найдено локально и добавлено в очередь."
    elif new_video_entry.status == "metadata_fetched":
        message = "Метаданные получены, видео готово к скачиванию."
    else:
        message = "Не удалось обработать видео. Проверьте статус в очереди."

    return AddVideoResponse(
        message=message, video_info=new_video_entry, queue_position=position
    )
...
async def play_next_video():
    ...
    return ActionSuccessResponse(
        message="Воспроизводится следующее видео", current_video=current_video
    )
...
async def play_previous_video():
    ...
    return ActionSuccessResponse(
        message="Воспроизводится предыдущее видео", current_video=current_video
    )
...
async def get_current_video_details():
    ...
    if current_video:
        return CurrentVideoResponse(
            message="Информация о текущем видео.", video_info=current_video
        )
    return CurrentVideoResponse(
        message="Очередь пуста или видео не выбрано.", video_info=None
    )
...
async def download_single_video(
    ...
    """Инициирует загрузку указанного видео из очереди."""
    ...
    if updated_video_entry.status == "downloaded":
        message = f"Видео '{updated_video_entry.title}' уже скачано."
    elif updated_video_entry.status == "downloading":
        message = f"Видео '{updated_video_entry.title}' уже скачивается."
    else:
        message = f"Начата загрузка видео '{updated_video_entry.title}'."
